...
contract Platform is Initializable, IPlatform, OwnableUpgradeable, ERC20Upgradeable, ReentrancyGuardUpgradeable {
    ...
    function openPositionWithoutFee(uint168 _tokenAmount, uint16 _maxCVI, uint8 _leverage) external override virtual returns (uint168 positionUnitsAmount, uint168 positionedTokenAmount) {
        require(noPremiumFeeAllowedAddresses[msg.sender]); // "Not allowed"
        return _openPosition(_tokenAmount, _maxCVI, 0, _leverage, false, false, false);
    }
    ...
    function openPositionWithoutPremiumFee(uint168 _tokenAmount, uint16 _maxCVI, uint8 _leverage) external override virtual returns (uint168 positionUnitsAmount, uint168 positionedTokenAmount) {
        require(noPremiumFeeAllowedAddresses[msg.sender]); // "Not allowed"
        return _openPosition(_tokenAmount, _maxCVI, 0, _leverage, false, false, true);
    }
    ...
    function openPositionWithoutVolumeFee(uint168 _tokenAmount, uint16 _maxCVI, uint16 _maxBuyingPremiumFeePercentage, uint8 _leverage) external override virtual returns (uint168 positionUnitsAmount, uint168 positionedTokenAmount) {
        require(noPremiumFeeAllowedAddresses[msg.sender]); // "Not allowed"
        return _openPosition(_tokenAmount, _maxCVI, _maxBuyingPremiumFeePercentage, _leverage, true, false, true);
    }
    ...
    function openPosition(uint168 _tokenAmount, uint16 _maxCVI, uint16 _maxBuyingPremiumFeePercentage, uint8 _leverage) external override virtual returns (uint168 positionUnitsAmount, uint168 positionedTokenAmount) {
        return _openPosition(_tokenAmount, _maxCVI, _maxBuyingPremiumFeePercentage, _leverage, true, true, true);
    }

    function closePositionWithoutFee(uint168 _positionUnitsAmount, uint16 _minCVI) external override returns (uint256 tokenAmount) {
        require(noPremiumFeeAllowedAddresses[msg.sender]); // "Not allowed"
        return _closePosition(_positionUnitsAmount, _minCVI, 0, false, false);
    }
    ...
    function closePositionWithoutVolumeFee(uint168 _positionUnitsAmount, uint16 _minCVI) external override virtual returns (uint256 tokenAmount) {
        require(noPremiumFeeAllowedAddresses[msg.sender]); // "Not allowed"
        return _closePosition(_positionUnitsAmount, _minCVI, 0, false, true);
    }
    ...
    function closePosition(uint168 _positionUnitsAmount, uint16 _minCVI, uint16 _maxClosingPremiumFeePercentage) external override virtual returns (uint256 tokenAmount) {
        return _closePosition(_positionUnitsAmount, _minCVI, _maxClosingPremiumFeePercentage, true, true);
    }

    function _closePosition(uint168 _positionUnitsAmount, uint16 _minCVI, uint16 _maxClosingPremiumFeePercentage, bool _chargeVolumeFee, bool _chargeCloseFee) private nonReentrant returns (uint256 tokenAmount) {
        ...
        uint256 lastCollateralRatio = calculateCollateralRatio();
...

        {
            ...
            } else if (_chargeCloseFee && feesCalculator.openPositionLPFeePercent() > 0) {
        ...
        uint256 closePositionFee = _chargeCloseFee ? positionBalance * feesCalculator.calculateClosePositionFeePercent(position.creationTimestamp, noLockPositionAddresses[msg.sender]) / MAX_FEE_PERCENTAGE : 0;
    ...
    function setAddressSpecificParameters(address _holderAddress, bool _shouldLockPosition, bool _noPremiumFeeAllowed, bool _increaseSharedPoolAllowed, bool _isLiquidityProvider) external override onlyOwner {
    ...
    function calculateLatestTurbulenceIndicatorPercent() external view override returns (uint16) {
        ...
        IFeesCalculator.SnapshotUpdate memory updateData = 
            feesCalculator.updateSnapshots(latestSnapshotTimestamp, cviSnapshots[block.timestamp], cviSnapshots[latestSnapshotTimestamp], latestOracleRoundId, calculateCollateralRatio());
    ...
    function _openPosition(uint168 _tokenAmount, uint16 _maxCVI, uint168 _maxBuyingPremiumFeePercentage, uint8 _leverage, bool _chargePremiumFee, bool _chargeVolumeFee, bool _chargeOpenFee) internal nonReentrant returns (uint168 positionUnitsAmount, uint168 positionedTokenAmount) {
        ...
        locals.openPositionFee = _chargeOpenFee ? _tokenAmount * _leverage * locals.openPositionFeePercent / MAX_FEE_PERCENTAGE : 0;
        ...
        if (_chargePremiumFee) {
            locals.lastCollateralRatio = calculateCollateralRatio();
        ...
        } else if (_chargeOpenFee && feesCalculator.openPositionLPFeePercent() > 0) {
    ...
    function updateSnapshots(bool _canPurgeLatestSnapshot) private returns (uint16 latestCVIValue, uint256 latestSnapshot, uint256 latestCVIValueTimestamp) {
        ...
        IFeesCalculator.SnapshotUpdate memory updateData = 
            feesCalculator.updateSnapshots(latestTimestamp, cviSnapshots[block.timestamp], cviSnapshots[latestTimestamp], latestOracleRoundId, calculateCollateralRatio());
    ...
    function calculateCollateralRatio() private view returns (uint256) {
        return totalLeveragedTokensAmount > 0 ? totalPositionUnitsAmount * PRECISION_DECIMALS / totalLeveragedTokensAmount : 0;
    }
