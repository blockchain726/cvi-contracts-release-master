...
interface IPlatform {
    ...
    function openPositionWithoutFee(uint168 tokenAmount, uint16 maxCVI, uint8 leverage) external returns (uint168 positionUnitsAmount, uint168 positionedTokenAmount);
    ...
    function closePositionWithoutFee(uint168 positionUnitsAmount, uint16 minCVI) external returns (uint256 tokenAmount);
