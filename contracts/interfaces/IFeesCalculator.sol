...
interface IFeesCalculator {
    ...
    function calculateSingleUnitFundingFee(CVIValue[] memory cviValues, uint256 collateralRatio) external view returns (uint256 fundingFee);
    ...
    function updateSnapshots(uint256 latestTimestamp, uint256 blockTimestampSnapshot, uint256 latestTimestampSnapshot, uint80 latestOracleRoundId, uint256 collateralRatio) external view returns (SnapshotUpdate memory snapshotUpdate);
