...
contract FeesCalculator is IFeesCalculator, Ownable {
    ...
    uint16 private constant MAX_FUNDING_FEE_CVI_THRESHOLD = 50;
    ...
    function calculateSingleUnitFundingFee(CVIValue[] memory _cviValues, uint256 _collateralRatio) public override view returns (uint256 fundingFee) {
        for (uint8 i = 0; i < _cviValues.length; i++) {
            (uint256 currFundingFee,) = calculateSingleUnitPeriodFundingFee(_cviValues[i], _collateralRatio);
            fundingFee = fundingFee + currFundingFee;
        }
    }

    function updateSnapshots(uint256 _latestTimestamp, uint256 _blockTimestampSnapshot, uint256 _latestTimestampSnapshot, uint80 _latestOracleRoundId, uint256 _collateralRatio) external override view returns (SnapshotUpdate memory snapshotUpdate) {
        ...
        snapshotUpdate.singleUnitFundingFee = calculateSingleUnitFundingFee(cviValues, _collateralRatio);
