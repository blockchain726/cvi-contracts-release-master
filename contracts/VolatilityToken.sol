...
contract VolatilityToken is Initializable, IVolatilityToken, IRequestManager, ReentrancyGuardUpgradeable, ElasticToken {
    ...
    function fulfillCollateralizedMintRequest(uint256 _requestId) public virtual override returns (uint256 tokensMinted, uint256 shortTokensMinted) {
        Request memory request = requests[_requestId];
        PreFulfillResults memory results = preFulfillRequest(_requestId, request, MINT_REQUEST_TYPE, true, false);

        if (!results.wasLiquidated) {
            subtractTotalRequestAmount(request.tokenAmount);
            deleteRequest(_requestId);

            if (results.shouldAbort) {
                token.safeTransfer(msg.sender, request.tokenAmount * request.maxRequestFeesPercent / MAX_PERCENTAGE + results.timeDelayFees);
            } else {
                feesCollector.sendProfit(results.timeDelayFees + results.fulfillFees, IERC20(address(token)));
                (tokensMinted, shortTokensMinted) = mintCollateralizedTokens(_requestId, results.mintAmount, results.depositAmount);
            }

            emit FulfillRequest(_requestId, request.owner, results.fulfillFees, results.shouldAbort, request.useKeepers, false, msg.sender);
        }
    }
    ...
    function mintTokens(uint168 tokenAmount) external override returns (uint256 mintedTokens) {
        ...
        (mintedTokens,) = mintTokens(0, msg.sender, tokenAmount, false, MAX_PERCENTAGE, false, false);
    }
    ...
    function submitRequest(uint8 _type, uint168 _tokenAmount, uint32 _timeDelay, bool _useKeepers, uint16 _maxBuyingPremiumFeePercentage) private nonReentrant returns (uint requestId) {
        ...
        emit SubmitRequest(requestId, _type, msg.sender, _tokenAmount, _type == BURN_REQUEST_TYPE ? underlyingToValue(locals.timeDelayFeeAmount) : locals.timeDelayFeeAmount, targetTimestamp, _useKeepers);
    }
    ...
    function preFulfillRequest(uint256 _requestId, Request memory _request, uint8 _expectedType, bool _isCollateralized, bool _keepersCalled) private returns (PreFulfillResults memory results) {
        ...
        if (requestFeesCalculator.isLiquidable(_request)) {
            ...
            if (_request.requestType == MINT_REQUEST_TYPE) {
                ...
                if (_isCollateralized) {
                    bool isPossible;
                    (results.mintAmount, results.depositAmount, isPossible) = calculateCollateralizedAmounts(results.amountToFulfill);

                    if (!isPossible) {
                        return PreFulfillResults(0, 0, results.timeDelayFees, 0, false, 0, 0, true, 0);
                    }
                }
            }
    ...
    function _fulfillMintRequest(uint256 _requestId, Request memory _request, uint16 _maxBuyingPremiumFeePercentage, bool _keepersCalled) private returns (uint256 tokensMinted, bool success) {
        PreFulfillResults memory results = preFulfillRequest(_requestId, _request, MINT_REQUEST_TYPE, false, _keepersCalled);

        if (!results.wasLiquidated) {
            (tokensMinted, success) = mintTokens(_requestId, _request.owner, results.amountToFulfill, false, _maxBuyingPremiumFeePercentage, _request.useKeepers && _keepersCalled, true);
            ...
            if (success) {
                ...
                emit FulfillRequest(_requestId, _request.owner, results.fulfillFees + results.keepersFee, false, _request.useKeepers, _keepersCalled, msg.sender);
            }
        }
    }
    ...
    function _fulfillBurnRequest(uint256 _requestId, Request memory _request, bool _keepersCalled) private nonReentrant returns (uint256 tokensReceived) {
        PreFulfillResults memory results = preFulfillRequest(_requestId, _request, BURN_REQUEST_TYPE, false, _keepersCalled);
        ...
        if (!results.wasLiquidated) {
            ...
            emit FulfillRequest(_requestId, _request.owner, fulfillFees, false, _request.useKeepers, _keepersCalled, msg.sender);
        }
    }

    function mintTokens(uint256 _requestId, address owner, uint168 _tokenAmount, bool _isCollateralized, uint16 _maxBuyingPremiumFeePercentage, bool _catchRevert, bool _chargeOpenFee) private returns (uint256 tokensMinted, bool success) {
        uint256 balance = 0;
        ...
        (, uint256 positionedTokenAmount, bool transactionSuccess) = openPosition(_tokenAmount, !_isCollateralized, _maxBuyingPremiumFeePercentage, _catchRevert, _chargeOpenFee);
        ...
        if (transactionSuccess) {
            ...
            if (!_isCollateralized) {
                emit Mint(_requestId, owner, _tokenAmount, positionedTokenAmount, tokensMinted);
            }

            require(tokensMinted > 0, "Too few tokens");

            _mint(owner, tokensMinted);
            success = true;
        }
    }
    ...
    function burnTokens(uint256 _requestId, address _owner, uint168 _tokenAmount, uint16 _timeDelayFeesPercentage, uint16 _fulfillFeesPercentage, bool _hasKeepersFee, bool _chargeCloseFee) private returns (uint256 tokensReceived, uint256 fulfillFees) {
        uint256 tokensBeforeFees = _burnTokens(_tokenAmount, _chargeCloseFee);
        ...
        emit Burn(_requestId, _owner, tokensBeforeFees, tokensReceived, _tokenAmount);
    }

    function _burnTokens(uint256 _tokenAmount, bool _chargeCloseFee) private returns (uint256 tokensReceived) {
        ...
        if (positionUnits > 0) {
            tokensReceived = _chargeCloseFee ? 
                platform.closePositionWithoutVolumeFee(uint168(positionUnits), 1) :
    ...
    function mintCollateralizedTokens(uint256 _requestId, uint168 _mintAmount, uint168 _depositAmount) private returns (uint256 tokensMinted, uint256 shortTokensMinted) {
        if (_depositAmount > 0) {
        ...
        bool success;
        (tokensMinted, success) = mintTokens(_requestId, msg.sender, _mintAmount, true, 0, false, true);

        if (success) {
            emit CollateralizedMint(_requestId, msg.sender, _mintAmount + _depositAmount, tokensMinted, shortTokensMinted);
        }
    }
    ...
    function _liquidateRequest(uint256 _requestId, Request memory _request) private returns (uint256 findersFeeAmount) {
        ...
        emit LiquidateRequest(_requestId, _request.requestType, _request.owner, msg.sender, findersFeeAmount, _request.useKeepers);
    }
    ...
    function openPosition(uint168 _amount, bool _withPremiumFee, uint16 _maxBuyingPremiumFeePercentage, bool _catchRevert, bool _chargeOpenFee) private returns (uint168 positionUnitsAmount, uint168 positionedTokenAmount, bool transactionSuccess) {
        transactionSuccess = true;
        require(!_catchRevert || (_catchRevert && _withPremiumFee)); // Not allowed

        if (_catchRevert) {
            (bool success, bytes memory returnData) = 
                address(platform).call(abi.encodePacked(platform.openPositionWithoutVolumeFee.selector, abi.encode(_amount, platform.maxCVIValue(), _maxBuyingPremiumFeePercentage, leverage)));

            if (success) {
                (positionUnitsAmount, positionedTokenAmount) = abi.decode(returnData, (uint168, uint168));
            } else {
                transactionSuccess = false;
            }
        } else {
            (positionUnitsAmount, positionedTokenAmount) = !_chargeOpenFee ? 
                platform.openPositionWithoutFee(_amount, platform.maxCVIValue(), leverage) : 
                    (_withPremiumFee ? 
