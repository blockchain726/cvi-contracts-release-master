...
const calculateCollateralizedMintAmount = async (amount, cviValue, margin, snapshot) => {
  const openFees = (await this.feesCalculator.openPositionFeePercent()).add(
    await this.feesCalculator.openPositionLPFeePercent(),
  )
  ...
  const mintAmount = new BN(-1)
    .mul(
      cviValue
        ...
        .sub(margin.mul(gain).mul(getContracts().maxCVIValue).mul(MAX_FEE_PERCENTAGE))
        .sub(cviValue.mul(depositFees).mul(amount))
        .add(cviValue.mul(MAX_FEE_PERCENTAGE).mul(amount)),
    )
    .div(
      cviValue
        .mul(depositFees)
        .sub(cviValue.mul(MAX_FEE_PERCENTAGE))
        .add(cviValue.mul(margin).mul(MAX_FEE_PERCENTAGE))
        .sub(margin.mul(getContracts().maxCVIValue).mul(MAX_FEE_PERCENTAGE))
        .sub(cviValue.mul(margin).mul(margin).mul(openFees))
        .add(margin.mul(margin).mul(getContracts().maxCVIValue).mul(openFees)),
    )
    .sub(new BN(1))
  ...
  const maxPositionUnits = gain
    .add(mintAmount)
    .sub(mintAmount.mul(margin).mul(openFees).div(MAX_FEE_PERCENTAGE))
    .mul(margin)
    .mul(getContracts().maxCVIValue.sub(cviValue))
    .div(cviValue)
...
const fulfillAndValidate = async (
  ...
  let afterContractTokens = getContracts().isETH
    ? await balance.current(this.volToken[margin.toString()].address, 'wei')
    : await getContracts().token.balanceOf(this.volToken[margin.toString()].address)
  let afterContractVolTokens = await this.volToken[margin.toString()].balanceOf(
    this.volToken[margin.toString()].address,
  )
  ...
  for (i = 0; i < requestIds.length; i++) {
    ...
    afterVolTokensBalances[account] = await this.volToken[margin.toString()].balanceOf(account)
  }
  ...
  if (isMultiple) {
    ...
    const isMint = this.state[margin.toString()].requests[requestIds[0]].requestType == MINT_REQUEST_TYPE

    if (isMint) {
      if (isCollateralized) {
        const result = await this.volToken[margin.toString()].fulfillCollateralizedMintRequest.call(requestIds[0], {
          from: accounts[0],
        })
        mintedTokensCall = result.tokensMinted
        lpTokensCall = result.shortTokensMinted
      } else {
        const result = await this.volToken[margin.toString()].fulfillMintRequest.call(
          requestIds[0],
          maxOpenPremiumFeePercentage,
          keepersCalled,
      ...
      tx = isCollateralized
        ? await this.volToken[margin.toString()].fulfillCollateralizedMintRequest(requestIds[0], { from: accounts[0] })
        : await this.volToken[margin.toString()].fulfillMintRequest(
            requestIds[0],
            maxOpenPremiumFeePercentage,
            keepersCalled,
            { from: keepersCalled ? keeper : accounts[0] },
      ...
      tokensReceivedCall = await this.volToken[margin.toString()].fulfillBurnRequest.call(
        requestIds[0],
        keepersCalled,
        { from: keepersCalled ? keeper : accounts[0] },
      )
      ...
      tx = await this.volToken[margin.toString()].fulfillBurnRequest(requestIds[0], keepersCalled, {
        from: keepersCalled ? keeper : accounts[0],
      })
  ...
  for (i = 0; i < requestIds.length; i++) {
    ...
    if (!isMint) {
      ...
      if (!isMultiple) {
        const result = await calculateBurnAmount(this.state, tokensAmount, timestampCall, margin, snapshotCall)
      ...
      const {
        ...
        positionBalance,
        marginDebt,
      } = await calculateBurnAmount(this.state, tokensAmount, timestamp, margin)
      ...
      const expectedFulfillRequestEvent = {
        ...
        keepersCalled,
        fulfiller: isMultiple ? this.requestFulfiller[margin.toString()].address : keepersCalled ? keeper : account,
      }
      ...
      const mintAmount = isCollateralized
        ? await calculateCollateralizedMintAmount(fulfillAmount, cviValue, new BN(margin), snapshot)
        : fulfillAmount
      ...
      const mintAmountCallExpected = isCollateralized
        ? await calculateCollateralizedMintAmount(fulfillAmountCall, cviValue, new BN(margin), snapshotCall)
        : fulfillAmountCall
      ...
      if (isCollateralized) {
        if (mintAmount.lt(new BN(0)) || depositAmount.lt(new BN(0))) {
          ...
          isAborted = true
          this.state.totalFundingFees = previousTotalFundingFees
        ...
        afterLPTokensBalances[account] = afterLPTokensBalances[account].add(isAborted ? new BN(0) : lpTokens)
        ...
        if (!isAborted) {
          ...
          await expectEvent.inTransaction(tx.tx, getContracts().platform, 'Deposit', {
            account: this.volToken[margin.toString()].address,
      ...
      const {
        ...
        positionUnits,
      } = await calculateOpenPositionAmounts(
        this.state,
        timestamp,
        mintAmount,
        isCollateralized ? ONLY_OPEN_FEES : ONLY_COLLATERAL_PREMIUM,
        margin,
      )
      ...
      if (isCollateralized) {
        if (!isAborted) {
          await expectEvent(tx, 'CollateralizedMint', {
            requestId,
            account,
            tokenAmount: fulfillAmount,
            mintedTokens: volTokens,
            mintedShortTokens: lpTokens,
          })
        }
      } else {
        ...
        if (shouldFulfill) {
          if (isMultiple) {
            await expectEvent.inTransaction(tx.tx, this.volToken[margin.toString()], 'Mint', expectedMintEvent)
          } else {
      ...
      if (!isAborted && shouldFulfill) {
        if (isMerge) {
          ...
          this.state.sharedPool = this.state.sharedPool
            .sub(positionBalance)
            .sub(marginDebt)
            .add(positionBalance.add(openPositionTokensMinusFees).mul(new BN(margin)))
            .add(openPositionPremiumFees)
          this.state.totalMarginDebt = this.state.totalMarginDebt
            .sub(marginDebt)
            .add(positionBalance.add(openPositionTokensMinusFees).mul(new BN(margin).sub(new BN(1))))
    ...
    if (shouldFulfill || !isMint) {
      if (isMint) {
        this.state[margin.toString()].totalRequestsAmount =
          this.state[margin.toString()].totalRequestsAmount.sub(tokensAmount)
      }

      this.state[margin.toString()].keepersFeeVaultBalance =
        this.state[margin.toString()].keepersFeeVaultBalance.add(keepersFee)
      removeRequest(requestId, margin)
    }
  }

  const afterActualContractTokens = await getContracts().token.balanceOf(this.volToken[margin.toString()].address)
  ...
  for (i = 0; i < requestIds.length; i++) {
    ...
    const currAcountVolTokensBalance = await this.volToken[margin.toString()].balanceOf(account)
...
const submitAndValidate = async (
  ...
  if (requestType === BURN_REQUEST_TYPE) {
    ...
    await this.volToken[margin.toString()].approve(
      this.volToken[margin.toString()].address,
  ...
  if (requestType === MINT_REQUEST_TYPE) {
    if (useKeepers) {
      tx = await this.volToken[margin.toString()].submitKeepersMintRequest(
        tokensAmount,
        delayTime,
    ...
    if (useKeepers) {
      tx = await this.volToken[margin.toString()].submitKeepersBurnRequest(tokensAmount, delayTime, { from: owner })
    } else {
  ...
  if (requestType === BURN_REQUEST_TYPE) {
    ...
    this.state[margin.toString()].totalRequestsAmount =
      this.state[margin.toString()].totalRequestsAmount.add(tokensAmount)
  }
  ...
  await expectEvent(tx, 'SubmitRequest', {
    requestId: new BN(this.state[margin.toString()].nextRequestId),
  ...
  const request = await this.volToken[margin.toString()].requests(requestId)
...
for (let margin of [1]) {
  ...
  describe(`VolatilityToken (margin = ${margin})`, () => {
    ...
    it('reverts when fulfilling collateralized mint reuqests of different owner', async () => {
      const { requestId } = await submitAndValidate(MINT_REQUEST_TYPE, new BN(1000), 2 * SECONDS_PER_HOUR, bob, margin)
      await expectRevert.unspecified(
        this.volToken[margin.toString()].fulfillCollateralizedMintRequest(requestId, { from: alice }),
      )
      await expectRevert.unspecified(
        this.volToken[margin.toString()].fulfillCollateralizedMintRequest(requestId, { from: admin }),
      )
    })
    ...
    it('reverts when fulfilling reuqests with an invalid id', async () => {
      ...
      await expectRevert.unspecified(
        this.volToken[margin.toString()].fulfillCollateralizedMintRequest(6, { from: bob }),
      )
    ...
    it('reverts when fulfilling collateralized mint reuqests of other types', async () => {
      ...
      await expectRevert(
        this.volToken[margin.toString()].fulfillCollateralizedMintRequest(burnRequestId, { from: bob }),
        'Wrong request type',
      )
    })
    ...
    it('catches revert when fulfilling mint request by keepers when max premium fee is exceeded (transaction succeeds, but fulfill is not done)', async () => {
      await depositAndValidate(this.state, margin * 2000 * 2, bob) // Small deposit to cause premium fee
    ...
    it('catches revert when fulfilling mint request by keepers when not enough platform liquidity (transaction succeeds, but fulfill is not done)', async () => {
      await depositAndValidate(this.state, toUSD(margin * 600), bob)
    ...
    it.skip('mints tokens properly collateralized without charging volume fee', async () => {})
    ...
    it.skip('mints tokens properly collateralized when formula numerator is negative', async () => {
      // NOTE: Calculate second mint that is possible to cover but negative numerator (is there such?)

      // For numerator to be negative, token amount must be small, and gain must be very high
      await this.fakePriceProvider.setPrice(toCVI(10000))
      await submitMintFulfillAndValidate(toBN(50000, 6), SECONDS_PER_HOUR, bob, margin, SECONDS_PER_HOUR, true)
      await this.fakePriceProvider.setPrice(toCVI(20000))
      await submitMintFulfillAndValidate(toBN(10000, 6), SECONDS_PER_HOUR, bob, margin, SECONDS_PER_HOUR, true)
    })
    ...
    it.skip('fulfills collateralized mint request properly, when rebase occurs between request submit and fulfill', async () => {})
    ...
    it.skip('liquidates collateralized mint request properly, when rebase occurs between request submit and liquidation', async () => {})
    ...
    const validateCalculateMint = async (tokensAmount, margin, timeWindow, isKeepers, requestId = 0) => {
      const result = await getContracts().platformHelper.calculatePreMintAmounts(
        this.volToken[margin.toString()].address,
        isKeepers,
        requestId,
        tokensAmount,
      ...
      let penaltyPercentage = toBN(0)
      if (!isKeepers) {
        const request = this.state[margin.toString()].requests[requestId]
        penaltyPercentage = latestTimestamp.gte(request.targetTimestamp)
          ? toBN(0)
          : calculateTimePenaltyFeePercentage(latestTimestamp, request.requestTimestamp, request.targetTimestamp)
      }

      const penaltyFees = isKeepers ? toBN(0) : tokensAmount.mul(penaltyPercentage).div(MAX_FEE_PERCENTAGE)
      const keepersFee = isKeepers ? calculateKeepersFee(tokensAmount) : toBN(0)

      const mintAmount = tokensAmount.sub(penaltyFees).sub(keepersFee).sub(timeWindowFee)
      ...
      //TODO: Get as constant
      expect(result.buyingPremiumFeePercentage).to.be.bignumber.equal(premiumPercentage.add(toBN(15)))
    ...
    const validateCalculateBurn = async (tokensAmount, margin, timeWindow, isKeepers, requestId = 0) => {
      const result = await getContracts().platformHelper.calculatePreBurnAmounts(
        this.volToken[margin.toString()].address,
        isKeepers,
        requestId,
        tokensAmount,
      ...
      const { tokensReceived, closeFees } = await calculateBurnAmount(this.state, tokensAmount, latestTimestamp, margin, snapshot)

      const timeWindowFeePercentage = calculateTimeDelayFeePercentage(timeWindow)
      const timeWindowFee = tokensReceived.mul(timeWindowFeePercentage).div(MAX_FEE_PERCENTAGE)
      ...
      const penaltyFees = isKeepers ? toBN(0) : tokensReceived.mul(penaltyPercentage).div(MAX_FEE_PERCENTAGE)
      const keepersFee = isKeepers ? calculateKeepersFee(tokensReceived) : toBN(0)

      const tokensReceivedAfterFees = tokensReceived.sub(penaltyFees).sub(keepersFee).sub(timeWindowFee)

      expect(result.expectedUSDCAmount).to.be.bignumber.equal(tokensReceivedAfterFees)
      expect(result.netBurnAmount).to.be.bignumber.equal(tokensAmount.mul(tokensReceivedAfterFees).div(tokensReceived.add(closeFees)))
    }

    it.only('helper calculates supposed minted tokens properly by keepers', async () => {
      await depositAndValidate(this.state, toUSD(margin * 5000 * 2), bob)

      const amount = toUSD(1000)

      // First mint ever
      await validateCalculateMint(amount, margin, MIN_TIME_DELAY, true, undefined)
      await validateCalculateMint(amount, margin, MAX_TIME_DELAY, true, undefined)
    ...
    it.only('helper calculates supposed minted tokens properly not by keepers', async () => {
      await depositAndValidate(this.state, toUSD(margin * 5000 * 2), bob)
      ...
      // First mint ever
      await validateCalculateMint(amount, margin, MIN_TIME_DELAY, false, requestId)
      await validateCalculateMint(amount, margin, MAX_TIME_DELAY, false, requestId)

      await fulfillMintAndValidate(requestId, request, timeDelayFee, bob, margin)

      const { requestId: requestId2 } = await submitAndValidate(
        MINT_REQUEST_TYPE,
        amount,
        2 * SECONDS_PER_HOUR,
        bob,
      ...
      // Second mint
      await validateCalculateMint(amount, margin, MIN_TIME_DELAY, false, requestId2)
      await validateCalculateMint(amount, margin, MAX_TIME_DELAY, false, requestId2)
    })

    it.only('helper calculates supposed burned tokens properly by keepers', async () => {
      await depositAndValidate(this.state, toUSD(margin * 10000 * 2), alice)
      await submitMintFulfillAndValidate(toUSD(1000), 2 * SECONDS_PER_HOUR, bob, margin)

      const amount = await this.volToken[margin.toString()].balanceOf(bob)

      await validateCalculateBurn(amount, margin, MIN_TIME_DELAY, true, undefined)
      await validateCalculateBurn(amount, margin, MAX_TIME_DELAY, true, undefined) // No fulfill fees
      await validateCalculateBurn(amount.div(toBN(2)), margin, MIN_TIME_DELAY, true, undefined) // Partial burn
    })

    it.only('helper calculates supposed burned tokens properly not by keepers', async () => {
      await depositAndValidate(this.state, toUSD(margin * 10000 * 2), alice)
      await submitMintFulfillAndValidate(toUSD(1000), 2 * SECONDS_PER_HOUR, bob, margin)
      ...
      await validateCalculateBurn(amount, margin, MIN_TIME_DELAY, false, requestId)
      await validateCalculateBurn(amount, margin, MAX_TIME_DELAY, false, requestId) // No fulfill fees

      //TODO: Fix
      //await validateCalculateBurn(amount.div(toBN(2)), margin, MIN_TIME_DELAY, true, requestId) // Partial burn
    })
  })
}
