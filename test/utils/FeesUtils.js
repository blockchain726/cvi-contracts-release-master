...
const calculateSingleUnitFeeRatePercent = (cviValue, period, collateral) => {
...
const calculateSingleUnitFee = (cviValue, period, collateral) => {
  ...
  return new BN(RATIO_DECIMALS_BN)
    .mul(toBN(cviValue))
    .mul(calculateSingleUnitFeeRatePercent(cviValue, period, collateral))
    .mul(new BN(period))
    .div(new BN(SECONDS_PER_DAY))
    .div(new BN(1000000))
    .div(new BN(22000)) //TOOD: Use max cvi
}
...
const calculatePremiumFee = (
  ...
  if (adjustedTimestamp !== undefined) {
    volumeFeePercentage = adjustedTimestamp.lt(timestamp.sub(FEE_TIME_WINDOW))
      ? midVolumeFee.mul(adjustedTimestamp.sub(timestamp.sub(TIME_WINDOW))).div(TIME_WINDOW.sub(FEE_TIME_WINDOW))
      : midVolumeFee.add(
          maxVolumeFee
            .sub(midVolumeFee)
            .mul(adjustedTimestamp.sub(timestamp.sub(FEE_TIME_WINDOW)))
            .div(FEE_TIME_WINDOW),
        )
  }
