...
const validateLPState = async state => {
  ...
  expect(await getContracts().feesCalculator.closeAdjustedVolumeTimestamp()).to.be.bignumber.equal(
    state.closeAdjustedTimestamp,
  )
  ...
  expect(await getContracts().platformHelper.dailyFundingFee(getContracts().platform.address)).to.be.bignumber.equal(
    calculateSingleUnitFeeRatePercent(cviValue, SECONDS_PER_DAY, expectedCollateral),
  )
}
...
const calculateNextAdjustedTimestamp = (adjustedTimestamp, timestamp, deltaCollateral) => {
  ...
  nextAdjustedTimestamp = nextAdjustedTimestamp.add(
    deltaCollateral.mul(TIME_WINDOW).div(MAX_FEE_DELTA_COLLATERAL.mul(PRECISION_DECIMALS).div(MAX_FEE)),
  )
...
const calculateOpenPositionAmounts = async (
  ...
  if (noPremiumFee !== NO_FEES && noPremiumFee !== ONLY_OPEN_FEES && noPremiumFee !== ONLY_COLLATERAL_PREMIUM) {
    adjustedTimestamp = calculateNextAdjustedTimestamp(
      state.adjustedTimestamp,
      timestamp,
      expectedCollateral.sub(lastCollateral),
    )
    if (saveState) {
  ...
  const openPositionPremiumFees =
    noPremiumFee === NO_FEES
      ? toBN(0)
      : noPremiumFee === ONLY_OPEN_FEES
      ? getFee(openPositionTokens.mul(new BN(leverage)), openPositionLPFeePercent)
      : getFee(openPositionTokens.mul(new BN(leverage)), premiumPercent)
...
const calculateMintAmount = async (
  ...
  if (state.positions[getContracts().volToken[margin.toString()].address] !== undefined) {
    if (lastLatestSnapshotTimestamp === undefined) {
      fundingFees = await calculateFundingFeesWithSnapshot(
        state,
        snapshot,
        getContracts().volToken[margin.toString()].address,
        state[margin.toString()].volTokenPositionUnits,
      )
    } else {
      fundingFees =
        state.positions[getContracts().volToken[margin.toString()].address] === undefined
          ? new BN(0)
          : (
  ...
  const currPosition = state.positions[getContracts().volToken[margin.toString()].address]

  let marginDebt = new BN(0)
  if (currPosition !== undefined) {
    marginDebt = margin
      .sub(new BN(1))
      .mul(state[margin.toString()].volTokenPositionUnits)
      .mul(currPosition.openCVIValue)
      .div(getContracts().maxCVIValue)
      .div(margin)
  }

  const positionBalance = (await calculatePositionBalance(state[margin.toString()].volTokenPositionUnits))
    .sub(marginDebt)
    .sub(fundingFees)

  // positionAmount / positionBalance = mintedToken / totalySupply => mintedTokens = positionAmount * totalSupply / positionBalance
  const volTokens = state[margin.toString()].volTokenSupply.eq(new BN(0))
...
const calculateBurnAmount = async (state, amount, time, margin, snapshot, chargeFees = true) => {
  ...
  const positionUnitsToBurn = amount
    .mul(state[margin.toString()].volTokenPositionUnits)
    .div(state[margin.toString()].volTokenSupply)
  ...
  const fundingFees =
    snapshot === undefined
      ? await calculateFundingFees(state, time, getContracts().volToken[margin.toString()].address, positionUnitsToBurn)
...
const openPositionWithoutVolumeFee = (tokens, cviValue, account, maxBuyingPremiumPercent = 1000, leverage = 1) => {
  if (getContracts().isETH) {
    return getContracts().platform.openPositionWithoutVolumeFeeETH(cviValue, maxBuyingPremiumPercent, leverage, {
      value: tokens,
      from: account,
      gasPrice: GAS_PRICE,
    })
  } else {
    return getContracts().platform.openPositionWithoutVolumeFee(tokens, cviValue, maxBuyingPremiumPercent, leverage, {
      from: account,
    })
  }
}
...
const callOpenPositionWithoutVolumeFee = (tokens, cviValue, account, maxBuyingPremiumPercent = 1000, leverage = 1) => {
  if (getContracts().isETH) {
    return getContracts().platform.openPositionWithoutVolumeFeeETH.call(cviValue, maxBuyingPremiumPercent, leverage, {
      value: tokens,
      from: account,
      gasPrice: GAS_PRICE,
    })
  } else {
    return getContracts().platform.openPositionWithoutVolumeFee.call(
      tokens,
      cviValue,
      maxBuyingPremiumPercent,
      leverage,
      { from: account },
    )
  }
...
const openPositionWithoutPremiumFee = (tokens, cviValue, account, leverage = 1) => {
  if (getContracts().isETH) {
    return getContracts().platform.openPositionWithoutPremiumFeeETH(cviValue, leverage, {
      value: tokens,
      from: account,
      gasPrice: GAS_PRICE,
    })
...
const callOpenPositionWithoutPremiumFee = (tokens, cviValue, account, leverage = 1) => {
  if (getContracts().isETH) {
    return getContracts().platform.openPositionWithoutPremiumFeeETH.call(cviValue, leverage, {
      value: tokens,
      from: account,
      gasPrice: GAS_PRICE,
    })
...
const closePosition = (positionUnits, cviValue, account, maxBuyingPremiumPercent = 1000) => {
  if (getContracts().isETH) {
    return getContracts().platform.closePosition(positionUnits, cviValue, maxBuyingPremiumPercent, {
      from: account,
      gasPrice: GAS_PRICE,
    })
  } else {
...
const callClosePosition = (positionUnits, cviValue, account, maxBuyingPremiumPercent = 1000) => {
  if (getContracts().isETH) {
    return getContracts().platform.closePosition.call(positionUnits, cviValue, maxBuyingPremiumPercent, {
      from: account,
      gasPrice: GAS_PRICE,
    })
  } else {
...
const closePositionWithoutVolumeFee = (positionUnits, cviValue, account) => {
  if (getContracts().isETH) {
    return getContracts().platform.closePositionWithoutVolumeFee(positionUnits, cviValue, {
      from: account,
      gasPrice: GAS_PRICE,
    })
  } else {
...
const callClosePositionWithoutVolumeFee = (positionUnits, cviValue, account) => {
  if (getContracts().isETH) {
    return getContracts().platform.closePositionWithoutVolumeFee.call(positionUnits, cviValue, {
      from: account,
      gasPrice: GAS_PRICE,
    })
  } else {
...
const openPositionAndValidate = async (
  ...
  const result =
    noPremiumFee === ONLY_COLLATERAL_PREMIUM
      ? await callOpenPositionWithoutVolumeFee(openPositionTokens, cviValue, account, 1000, leverage)
      : noPremiumFee === ONLY_OPEN_FEES
      ? await callOpenPositionWithoutPremiumFee(openPositionTokens, cviValue, account, leverage)
  ...
  const tx =
    noPremiumFee === ONLY_COLLATERAL_PREMIUM
      ? await openPositionWithoutVolumeFee(openPositionTokens, cviValue, account, 1000, leverage)
      : noPremiumFee === ONLY_OPEN_FEES
      ? await openPositionWithoutPremiumFee(openPositionTokens, cviValue, account, leverage)
...
const closePositionAndValidate = async (
  ...
  noVolumeFee = false,
  midVolumeFee = MID_VOLUME_FEE,
  maxVolumeFee = MAX_CLOSE_VOLUME_FEE,
) => {
  ...
  const result = noVolumeFee
    ? await callClosePositionWithoutVolumeFee(positionUnits, cviValue, account)
    : await callClosePosition(positionUnits, cviValue, account)

  const { latestTimestamp: timestampCall, snapshot: snapshotCall } = await updateSnapshots(state, false)

  const tx = noVolumeFee
    ? await closePositionWithoutVolumeFee(positionUnits, cviValue, account)
  ...
  let closeAdjustedTimestampCall = undefined
  if (!noVolumeFee && !shouldLiquidate) {
    closeAdjustedTimestampCall = calculateNextAdjustedTimestamp(
      state.closeAdjustedTimestamp,
      timestampCall,
      lastCollateral.sub(expectedCollateral),
    )
    closeAdjustedTimestamp = calculateNextAdjustedTimestamp(
      state.closeAdjustedTimestamp,
      timestamp,
      lastCollateral.sub(expectedCollateral),
    )
...
exports.ONLY_OPEN_FEES = ONLY_OPEN_FEES
