...
const beforeEachPlatform = async (maxCVIValue, maxTruncatedCVIValue) => {
  ...
  this.oracle = await CVIOracle.new(this.fakePriceProvider.address, this.fakeSanityPriceProvider.address, maxCVIValue, {
    from: admin,
  })
...
const setOracleTests = () => {
  ...
  it('reverts when getting truncated price that is zero because of bad max CVI', async () => {
    this.oracle = await CVIOracle.new(this.fakePriceProvider.address, this.fakeSanityPriceProvider.address, new BN(0), {
      from: admin,
    })
