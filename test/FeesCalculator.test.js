...
describe('FeesCalcaultor', () => {
  beforeEach(async () => {
    ...
    this.fakeOracle = await CVIOracle.new(this.fakePriceProvider.address, ZERO_ADDRESS, { from: admin })
  ...
  it('calculates single unit funding fee properly (not collateral dependant)', async () => {
    ...
    for (let i = 0; i < FUNDING_FEE_TESTS.length; i++) {
      let result = calculateSingleUnitFee(FUNDING_FEE_TESTS[i][0].cviValue, FUNDING_FEE_TESTS[i][0].period, 0)
      expect(await this.feesCalculator.calculateSingleUnitFundingFee(FUNDING_FEE_TESTS[i], 0)).to.be.bignumber.equal(
        result,
      )
