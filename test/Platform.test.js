...
const setPlatformTests = isETH => {
  ...
  it('opens a position with no volume fee properly (but with collateral fee)', async () => {
    ...
    const { volumeFeePercentage, premiumPercentage } = await openPositionAndValidate(
      this.state,
      toTokenAmount(1),
      alice,
      true,
  ...
  it('closes a position with no volume fee properly', async () => {
    ...
    await time.increase(SECONDS_PER_DAY.mul(toBN(3)))
    const { volumeFeePercentage } = await closePositionAndValidate(
      this.state,
      positionUnits,
      alice,
      undefined,
  ...
  it.skip('closes a position with no volume fee, but with LP close premium fee properly', async () => {})
  ...
  it('opens a position with no volume fee (adjusted timestamp is before h1)', async () => {
    ...
    const position = getPositionAmountByDeltaCollateral(
      MAX_FEE_DELTA_COLLATERAL.mul(TIME_WINDOW.sub(FEE_TIME_WINDOW)).div(toBN(2)).div(TIME_WINDOW),
      deposit,
      cviValue,
    )
  ...
  it('calculates volume fee correctly if fee at h1 is not zero', async () => {
    ...
    let position = getPositionAmountByDeltaCollateral(
      MAX_FEE_DELTA_COLLATERAL.mul(TIME_WINDOW.sub(FEE_TIME_WINDOW)).div(toBN(2)).div(TIME_WINDOW),
      deposit,
      cviValue,
    )
    ...
    position = getPositionAmountByDeltaCollateral(
      MAX_FEE_DELTA_COLLATERAL.mul(TIME_WINDOW.sub(FEE_TIME_WINDOW.div(toBN(2)))).div(TIME_WINDOW),
      deposit,
      cviValue,
    )
  ...
  it('grows volume fee by massive opening of positions one after the other', async () => {
    ...
    const position = FEE_TIME_WINDOW.div(new BN(4))
      .mul(MAX_FEE_DELTA_COLLATERAL)
      .mul(deposit)
      .div(TIME_WINDOW)
      .div(MAX_FEE)
      .mul(toBN(cviValue))
  ...
  it('zeroes volume fee if waiting until time window is over', async () => {
    ...
    const position = TIME_WINDOW.mul(new BN(2))
      .mul(MAX_FEE_DELTA_COLLATERAL)
      .mul(deposit)
      .div(TIME_WINDOW)
      .div(MAX_FEE)
    ...
    const positionNoFees = TIME_WINDOW.sub(FEE_TIME_WINDOW)
      .mul(MAX_FEE_DELTA_COLLATERAL)
      .mul(deposit)
      .div(TIME_WINDOW)
      .div(MAX_FEE)
  ...
  it('does not reduce volume fee on many small trades attack', async () => {
    ...
    const { volumeFeePercentage: bigVolumeFeePercentage } = await openPositionAndValidate(
      this.state,
      bigPosition,
      alice,
    )
    ...
    const { volumeFeePercentage: mediumVolumeFeePercentage } = await openPositionAndValidate(
      this.state,
      mediumPosition,
      alice,
    )
    expect(mediumVolumeFeePercentage).to.be.bignumber.at.least(MAX_VOLUME_FEE.div(volumeFeePercentage)) //TOOD: Is this correct?...
  })
  ...
  it('does not reduce close volume fee on many small trades attack', async () => {
    ...
    const { volumeFeePercentage: bigVolumeFeePercentage } = await closePositionAndValidate(
      this.state,
      allPositionUnits[21],
      alice,
    )
    ...
    const { volumeFeePercentage: mediumVolumeFeePercentage } = await closePositionAndValidate(
      this.state,
      allPositionUnits[0],
      alice,
    )
  ...
  it('changes volume fee adequately after withdrawing a large amount', async () => {
    ...
    const { volumeFeePercentage: smallVolumeFeePercentage } = await openPositionAndValidate(
      this.state,
      smallPosition,
      eve,
    )
  ...
  it('changes close volume fee adequately after depositing a large amount', async () => {
    ...
    const { volumeFeePercentage: smallVolumeFeePercentage } = await closePositionAndValidate(
      this.state,
      smallPositionUnits,
      eve,
    )
  ...
  it('charges higher fees for a big trade rather than parts-splitted trade of equal amount', async () => {
    ...
    const { volumeFeePercentage: volumeFeePercentageSingleTrade, positionUnits: positionUnits2 } =
      await openPositionAndValidate(this.state, bigPosition, carol)
    ...
    const { volumeFeePercentage: volumeFeePercentageFirstTrade } = await openPositionAndValidate(
      this.state,
      bigPosition.div(toBN(2)),
      carol,
    )

    await time.increase(SECONDS_PER_HOUR.div(toBN(6)))

    const { volumeFeePercentage: volumeFeePercentageSecondTrade } = await openPositionAndValidate(
      this.state,
      bigPosition.div(toBN(2)),
      carol,
    )
  ...
  it('charges higher close fees for a big trade rather than parts-splitted trade of equal amount', async () => {
    ...
    await closePositionAndValidate(this.state, positionUnits, alice)
    const { volumeFee: volumeFeeFirstTrade } = await closePositionAndValidate(
      this.state,
      positionUnitsTrade.div(toBN(2)),
      carol,
    )
    const { volumeFee: volumeFeeSecondTrade } = await closePositionAndValidate(
      this.state,
      positionUnitsTrade.div(toBN(2)),
      carol,
    )
  ...
  it.skip('reverts when trying to open a position with no fees without privilage', async () => {})

  it('reverts when trying to open a position with no premium fee without privilage', async () => {
    await depositAndValidate(this.state, 40000, bob)

    await expectRevert.unspecified(openPositionAndValidate(this.state, 1000, bob, undefined, ONLY_OPEN_FEES))
    await expectRevert.unspecified(openPositionAndValidate(this.state, 1000, alice, undefined, ONLY_OPEN_FEES))

    await this.platform.setAddressSpecificParameters(alice, true, true, false, { from: admin })

    await expectRevert.unspecified(openPositionAndValidate(this.state, 1000, bob, undefined, ONLY_OPEN_FEES))
    await openPositionAndValidate(this.state, 1000, alice, undefined, ONLY_OPEN_FEES)
  ...
  it('reverts when trying to close too many positional units', async () => {
    ...
    await expectRevert.unspecified(
      this.platform.closePosition(positionUnits.add(new BN(1)), 5000, 1000, { from: alice }),
    )
    await closePositionAndValidate(this.state, positionUnits, alice)
  })
  ...
  it('sets no lock address properly', async () => {
    ...
    await this.platform.setAddressSpecificParameters(alice, true, false, false, { from: admin })
    await this.platform.setAddressSpecificParameters(carol, false, false, false, { from: admin })
  ...
  it.skip('sets no premium fee allowed properly', async () => {
